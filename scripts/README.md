...
3. [`03-generate_corridor.R`](./03-generate_corridor.R): Run the corridor delineation and segmentation, using the input data retrieved (see point 2.). Note that only the segments are actually saved as output, since the corridor can be easily obtained from the segments via unioning of their geometries. The script carries out the delineations for a single city, taking the path to the input and output paths as command line arguments, e.g.:
  ...
  Run for all cities via the [`03-generate_corridor.bash`](./03-generate_corridor.bash) shell script:

  ```shell
  bash 03-generate_corridor.bash # Run delineation locally
  bash 03-generate_corridor.bash --slurm  # OR submit delineation task as a SLURM job
  ```
